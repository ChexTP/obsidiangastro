...
export const postOrder=async(req,res)=>{try{
  ...
  const templates=await model.listTemplates(req.tenantId);const items=[...requestedItems.map(item=>{const product=byId.get(item.productId);return product?prepareRequestedItem(item,product):null}),...requestedPreparations.map(item=>buildPreparation(item.ingredientIds,byId,templates))];
  if(items.some(item=>!item))return res.status(400).json({message:"Completa las opciones del combo y revisa las existencias disponibles"});
...
export const putOrder=async(req,res)=>{try{
  ...
  const templates=await model.listTemplates(req.tenantId);const items=[...requested.map(item=>{const product=byId.get(item.productId);return product?prepareRequestedItem(item,product,previous.get(item.productId),previousOptions):null}),...requestedPreparations.map(item=>buildPreparation(item.ingredientIds,byId,templates,previousIngredients))];
  if(items.some(item=>!item))return res.status(400).json({message:"Completa las opciones del combo y revisa las existencias disponibles"});

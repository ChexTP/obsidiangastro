...
export const postEmployee=async(req,res)=>{
  try{const email=req.body.email?.trim().toLowerCase(),displayName=req.body.displayName?.trim(),role=req.body.role,password=req.body.password;if(!email?.includes("@")||!displayName||!roles.includes(role)||typeof password!=="string"||password.length<8)return res.status(400).json({message:"Nombre, correo, rol y contraseña de al menos 8 caracteres son obligatorios"});if(role==="owner"&&req.membership.role!=="owner")return res.status(403).json({message:"Solo un propietario puede crear propietarios"});res.status(201).json({message:"Usuario creado y asociado al restaurante",data:await createEmployeeAccount({accessToken:req.accessToken,tenantId:req.tenantId,email,password,displayName,role})})}catch(error){const status=error.message?.toLowerCase().includes("limit")?409:400;res.status(status).json({message:"No fue posible crear el usuario",error:error.message})}
};

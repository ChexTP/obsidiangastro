...
export const listProducts = (tenantId) => query(supabaseAdmin.from("products").select("*,product_categories(name)").eq("tenant_id",tenantId).order("name"));
...
export const updateProduct = (tenantId,id,values) => query(supabaseAdmin.from("products").update(values).eq("tenant_id",tenantId).eq("id",id).select("*,product_categories(name)").single());
...
export const getDailyMenu = async (tenantId, serviceDate) => {
  ...
  const {data,error}=await supabaseAdmin.from("daily_menus")
    .select("*,daily_menu_items(*,products(*,product_categories(name)))")

...
export default function PrintOrderPage(){
  const{id}=useParams();const[search]=useSearchParams();const printType=search.get("tipo"),mode=printType==="comanda"?"kitchen":"receipt",paidReceipt=printType==="recibo";const[order,setOrder]=useState(null);const[profile,setProfile]=useState(null);const[error,setError]=useState("");const{memberships}=useAuth();
  ...
  return <main className={`thermal-page ${mode}`}>
    <div className="print-toolbar"><button onClick={()=>window.print()}>Imprimir ahora</button><span>{mode==="kitchen"?"Comanda de cocina":paidReceipt?"Recibo de pago":"Comprobante para cliente"}</span></div>
    <article className="thermal-paper">
      {mode==="receipt"?<>
        <header className="receipt-header"><h1>{restaurant}</h1>{profile?.tenant?.legal_name&&<p>{profile.tenant.legal_name}</p>}{profile?.tenant?.document_number&&<small>{profile.tenant.document_type} {profile.tenant.document_number}{profile.tenant.verification_digit?`-${profile.tenant.verification_digit}`:""}</small>}<p>{profile?.branch?.address}{profile?.branch?.city?` · ${profile.branch.city}`:""}</p>{profile?.tenant?.phone&&<small>Tel. {profile.tenant.phone}</small>}<p>{paidReceipt?"RECIBO DE PAGO":"Comprobante de venta"} · No. {order.order_number}</p></header>
        ...
        <div className="receipt-items"><div className="receipt-row receipt-label"><span>Producto</span><b>Total</b></div>{order.order_items.map(item=><div className="receipt-row" key={item.id}><span><strong>{Number(item.quantity)} × {item.product_name}</strong>{(item.selections||[]).map(selection=><small key={selection.optionId}>{selection.group}: {selection.name}</small>)}{item.notes&&<small>{item.notes}</small>}<small>{money(item.unit_price)} c/u</small></span><b>{money(Number(item.unit_price)*Number(item.quantity))}</b></div>)}</div>
        <div className="receipt-totals"><p><span>Subtotal alimentos</span><b>{money(order.subtotal)}</b></p>{Number(order.service_fee)>0&&<p><span>{order.service_type==="delivery"?"Costo de domicilio":"Costo de empaque"}</span><b>{money(order.service_fee)}</b></p>}<p className="grand-total"><span>{paidReceipt?"TOTAL PAGADO":"TOTAL"}</span><b>{money(order.total)}</b></p>{paidReceipt&&(order.payments||[]).map((payment,index)=><p key={`${payment.method}-${index}`}><span>{paymentLabels[payment.method]||payment.method}</span><b>{money(payment.amount)}</b></p>)}</div>
        ...
        <div className="kitchen-items">{order.order_items.map(item=><div key={item.id}><b>{Number(item.quantity)}</b><span><strong>{item.product_name}</strong>{(item.selections||[]).map(selection=><small key={selection.optionId}>{selection.group}: {selection.name}</small>)}{item.notes&&<small>NOTA: {item.notes}</small>}</span></div>)}</div>

...
import { addCashMovement, closeCash, getCash, getCashHistory, openCash } from "../../controllers/operationsController";
...
export default function CashPage() {
  ...
  const load = () => { setLoading(true); return Promise.all([getCash(), getCashHistory()]).then(([data, historyData]) => { setSession(data.session); setHistory(historyData.sessions || []); }).catch((failure) => setError(failure.message)).finally(() => setLoading(false)); };
  useEffect(() => { load(); }, []);
